...
public:
  ...
  //Added
  const std::vector<Matcher::p_match> Getp_matches(){return p_matched;}//This for me to get the matches (After RANSAC) and show the matching results
//  std::vector<Matcher::p_match>           test; //Added for debug

  //Added
  void AddNormals(std::vector<Eigen::Vector3d> normals){myNormals_ = normals;};
...
protected:
  ...
  double                        *J;          // jacobian
  double                        *J2;          // the second jacobian //Added
  ...
  //Added
  std::vector<Eigen::Vector3d> myNormals_ ;
