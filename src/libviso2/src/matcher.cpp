...
void Matcher::pushBack (uint8_t *I1,uint8_t* I2,int32_t* dims,const bool replace) {
  ...
  // copy images to byte aligned memory //Get current
...
void Matcher::matchFeatures(int32_t method, Matrix *Tr_delta,
  ...
  raw_matched_delete_.clear();//Added for DEBUG //Drawing features and matches in the objects
  ...
  if (param.multi_stage) {
    ...
    matching(m1p1,m2p1,m1c1,m2c1,n1p1,n2p1,n1c1,n2c1,p_matched_1,method,false,Tr_delta,bounds,1);

    // Saves the set of matches without any outliers removed: useful for DynSLAM where we care about
    // all SF points.
    raw_matches = p_matched_1;
    //Get all the raw flow
    raw_matches.insert(raw_matches.end(),raw_matched_delete_.begin(),raw_matched_delete_.end());
    ...
    matching(m1p2,m2p2,m1c2,m2c2,n1p2,n2p2,n1c2,n2c2,p_matched_2,method,true,Tr_delta,bounds);
  ...
  // single pass matching
    ...
    matching(m1p2,m2p2,m1c2,m2c2,n1p2,n2p2,n1c2,n2c2,p_matched_2,method,false,Tr_delta,bounds);
    if (param.refinement>0)
      refinement(p_matched_2,method);
    removeOutliers(p_matched_2,method);
  }
...
void Matcher::matching (int32_t *m1p,int32_t *m2p,int32_t *m1c,int32_t *m2c,
                        ...
                        std::vector<std::pair<std::pair<int,int>,std::pair<int,int>>> bounds,//Added
                        bool needRaw) {
  ...
  // method: quad matching
    ...
    for (i1p=0; i1p<n1p; i1p++) {
      ...
      v2p = *(m2p+step_size*i2p+1); //Matches between left and right in previous
      ...
      if (i1p2==i1p) {
        ...
        if (u1p>=u2p && u1c>=u2c) {
            ...
            //Added
            //remove matches that located on dynamic objects.
            //Remove features on the dynamic objects.
            bool inside = false;
            if(!bounds.empty()){
                for (size_t i_bound = 0;i_bound<bounds.size();i_bound++) {
                    auto bound = bounds[i_bound];
                    //bound.first for x0,x1
                    //bound.second for y0,y1
                    bool inside1 = u1c >= (bound).first.first && u1c <= (bound).first.second
                                  && v1c >= (bound).second.first && v1c <= (bound).second.second;
                    /*bool inside1 = u2c >= (bound).first.first && u2c <= (bound).first.second
                                  && v2c >= (bound).second.first && v2c <= (bound).second.second;
                                  */
                    inside = inside || inside1;
                    if(inside1 && needRaw)
                    {
                        raw_matched_delete_.push_back(Matcher::p_match(u1p,v1p,i1p,u2p,v2p,i2p,
                                                                       u1c,v1c,i1c,u2c,v2c,i2c));
                    } //For Drawing features and matches on the objects
                }
            }
            //-----------------------------------------------------------------------------------------
