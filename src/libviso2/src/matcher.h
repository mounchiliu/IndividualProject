...
public:
  ...
  void matchFeatures(int32_t method, Matrix *Tr_delta = 0, std::vector<std::pair<std::pair<int,int>,std::pair<int,int>>> bounds ={});
  ...
  //-------------------------------------------------------------------------------
  //Added
  //Raw matches that located on dynamic objects
  const std::vector<Matcher::p_match> GetDeletedRawMatches() const {return raw_matched_delete_;}
  //-------------------------------------------------------------------------------
...
private:
  ...
  void matching (int32_t *m1p,int32_t *m2p,int32_t *m1c,int32_t *m2c,
                 ...
                 bool needRaw = false);//Has modified //Added
  ...
  //Added
  //deleted matches
  std::vector<Matcher::p_match>  raw_matched_delete_;
};
