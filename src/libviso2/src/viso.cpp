...
bool VisualOdometry::updateMotion () {
  ...
  vector<double> tr_delta = estimateMotion(p_matched, initial_estimate); //Find motion according to matches
...
Matrix VisualOdometry::transformationVectorToMatrix (vector<double> tr) {
  ...
  //He use euler angle (which shoould specify an order when we do the rotation)
  //https://en.wikipedia.org/wiki/Rotation_formalisms_in_three_dimensions#Cayley%E2%80%93Klein_parameters
  Matrix Tr(4,4);
