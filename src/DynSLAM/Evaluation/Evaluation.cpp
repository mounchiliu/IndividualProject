...
void PrettyPrintStats(const string &label, const DepthFrameEvaluation &evals) {
  ...
  for (auto &eval : evals.evaluations) {
    cout << "[" << label << "] Evaluation on frame #" << evals.meta.frame_idx << ", delta max = "
         ...
         count++;
//    //Add
//    if(label == "Dynamic" && eval.fused_result.GetCorrectPixelRatio(missing_depths_are_errors)>0 && count == evals.evaluations.size()){
//        ofstream outputfile4("./record/Dynamic.txt",ios::app);

//        //reconstruction accuracy of dynamic map
//        outputfile4<<eval.fused_result.GetCorrectPixelRatio(missing_depths_are_errors)<<endl;
//    }
//    //End
