...
//    cout << "My new car egomotion: " << endl << *this->last_egomotion_ << endl;
  ...
  void SetPose(const Eigen::Matrix4f &new_pose) {
      //Update last egomotion
    *(this->last_egomotion_) = ItmToEigen(this->trackingState->pose_d->GetInvM()).inverse() * new_pose;//Back to the first frame that observe this object
     // For static scene, this function updates the egomotion.
     // new pose for new estimated camera pose (to current)
     // pose_d for previous camera pos
  ...
  void SetPoseInstance(const Eigen::Matrix4f &new_pose) {
      //Update last egomotion
    *(this->last_egomotion_) = ItmToEigen(this->trackingState->pose_d->GetInvM()).inverse()* new_pose;
     //to the previous frame before the previous frame
