      ...
      if (evaluated_frame_idx > 0) {
        ...
        switch(current_lidar_vis_) {
          case kNone:
            ...
            preview = dyn_slam_->GetStaticMapRaycastPreview( //Also render the dynamic objects
                pane_cam_->GetModelViewMatrix(),
//                pango_pose,
                static_cast<PreviewType>(current_preview_type_),
                enable_compositing);
            pane_texture_->Upload(preview, GL_RGBA, GL_UNSIGNED_BYTE);//static scene reconstruction
            pane_texture_->RenderToViewport(true);//render
            DrawPoses(time_ms);
...
//              lidar_vis_vertices_);
          ...
          //auto vis_mode = eval::SegmentedCallback::LidarAssociation::kDynamicReconstructed;
...
//        UploadCvTexture(this->depth_preview_buffer_, *pane_texture_, false, GL_SHORT);
      ...
      if (dyn_slam_->GetCurrentFrameNo() > 0 && preview_sf_->Get()) {   //Draw sparse flow
        ...
        if (tracker.HasTrack(visualized_object_idx_)) {
          ...
          PreviewSparseSF(instance_flow, object_view_);//render
...
//    font.Text("libviso2 scene flow preview").Draw(-0.90f, 0.89f);
 ...
 protected:
  ...
  void CreatePangolinDisplays() {    
    ...
    auto next_frame = [this]() {//When press next frame bottom.
      *(this->autoplay_) = false; //For initialize, process the first frame.
      this->ProcessFrame(); //Start to process frame and tracking and reconstruction.
    };
 ...
 private:
  ...
  //Added
  //pangolin::View trajectory_view_;
...
void BuildDynSlamKittiOdometry(const string &dataset_root,
  ...
  double downscale_factor = FLAGS_scale; //FLAGS_scale = 1, from the comment by author: Whether to run in reduced-scale mode. Used for experimental purposes.
                                                                                      //Requires the (odometry) sequence to have been preprocessed using the
                                                                                       //'scale_sequence.py' script.
  ...
  if (FLAGS_dataset_type == kKittiOdometry) {//Kitti Odomotry dataset dataset
  ...
  //2. Read all the calibration infomation we need.
  ...
  // Read all the calibration info we need. (intrinsic matrix)
  // HERE BE DRAGONS Make sure you're using the correct matrix for the grayscale and/or color cameras!
  ReadKittiOdometryCalibration(dataset_root + "/" + input_config.calibration_fname, //Get the address of the calibration matrix (calibration_fname = calib.txt)
  ...
  //some other parameters, not sure what do this parameter for.
  ...
  *input_out = new Input(
  ...
  DepthProvider *depth = new PrecomputedDepthProvider(
      *input_out,
      dataset_root + "/" + input_config.depth_folder, //precomputed-depth images
      input_config.depth_fname_format,//name format of the depth sequence
      input_config.read_depth, //whether read the depth sequence.
      frame_offset,
      input_config.min_depth_m,//Minimum depth to keep when computing depth maps
      input_config.max_depth_m //Maximum depth to keep when computing depth maps
  );
  (*input_out)->SetDepthProvider(depth);//link the input sequence with the pre-calculated depth sequence.
  ...
  //Setting of infiniTam
  ITMLibSettings *driver_settings = new ITMLibSettings();
  driver_settings->groundTruthPoseFpath = dataset_root + "/" + input_config.odometry_fname;//ground-truth-poses.txt(odometry sequence) or null for tracking sequence
  ...
  //Get results of segmented objects.
...
//  sf_params.inlier_threshold = 2.7;   // May be required for the hill sequence
  ...
  *dyn_slam_out = new DynSlam(//Initialize the system. //Current question
      driver,
      segmentation_provider,
      sparse_sf_provider,
      evaluation,
      input_shape,//size of the frame
...
int main(int argc, char **argv) {
  ...
  dynslam::Input *input;
  BuildDynSlamKittiOdometry(dataset_root, &dyn_slam, &input);//settings and initialize the system

  dynslam::gui::PangolinGui pango_gui(dyn_slam, input);//initialize the GUI based on the sequence.
  pango_gui.Run();


  //Added
  //File for saving trajectory of dynamic objects
  string s1 = "Save/CameraTrajectory";
  string s2 = "txt";
  const string &filename2 = s1+"."+s2;//one file for each instance
  //save
  ofstream f2;
  f2.open(filename2.c_str());
  f2 << fixed;
  //save trajectory of camera poses
  auto phist = dyn_slam->GetPoseHistory();
 for (int i = 0; i < static_cast<int>(phist.size()); ++i) {
     //convert to Quaternion
     //eigen matrix -> Quaternion

     //object world motion
     Eigen::Matrix4f motion = phist[i];//world to camera
     //Rotation matrix (camera to world)
     Eigen::Matrix3f R = motion.block(0,0,3,3).transpose();

     Eigen::Vector3f t = -R*motion.block(0,3,3,1);  // we need the translation of pose_cam_to_world to draw the trajectory

     //Matric convert to Quaternion
     Eigen::Quaternionf q_R = Eigen::Quaternionf((R));
    ...
    f2 <<setprecision(1) << i << " " <<  setprecision(9)
      << t[0] << " " << t[1] << " " << t[2] << " "
      << q_v[0] << " " << q_v[1] << " " << q_v[2] << " " << q_v[3] << endl;
